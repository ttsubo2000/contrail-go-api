...
const (
	...
	telemetry_profile_grpc_profile_refs
	telemetry_profile_netconf_profile_refs
	telemetry_profile_snmp_profile_refs
...
type TelemetryProfile struct {
	...
	grpc_profile_refs contrail.ReferenceList
	netconf_profile_refs contrail.ReferenceList
	snmp_profile_refs contrail.ReferenceList
...
func (obj *TelemetryProfile) readGrpcProfileRefs() error {
        if !obj.IsTransient() &&
                !obj.valid[telemetry_profile_grpc_profile_refs] {
                err := obj.GetField(obj, "grpc_profile_refs")
                if err != nil {
                        return err
                }
        }
...
func (obj *TelemetryProfile) GetGrpcProfileRefs() (
        contrail.ReferenceList, error) {
        err := obj.readGrpcProfileRefs()
        if err != nil {
                return nil, err
        }
        return obj.grpc_profile_refs, nil
}

func (obj *TelemetryProfile) AddGrpcProfile(
        rhs *GrpcProfile) error {
        err := obj.readGrpcProfileRefs()
        if err != nil {
                return err
        }

        if !obj.modified[telemetry_profile_grpc_profile_refs] {
                obj.storeReferenceBase("grpc-profile", obj.grpc_profile_refs)
        }

        ref := contrail.Reference {
                rhs.GetFQName(), rhs.GetUuid(), rhs.GetHref(), nil}
        obj.grpc_profile_refs = append(obj.grpc_profile_refs, ref)
        obj.modified[telemetry_profile_grpc_profile_refs] = true
        return nil
}

func (obj *TelemetryProfile) DeleteGrpcProfile(uuid string) error {
        err := obj.readGrpcProfileRefs()
        if err != nil {
                return err
        }
        ...
        for i, ref := range obj.grpc_profile_refs {
                if ref.Uuid == uuid {
                        obj.grpc_profile_refs = append(
                                obj.grpc_profile_refs[:i],
                                obj.grpc_profile_refs[i+1:]...)
                        break
                }
        }
        obj.modified[telemetry_profile_grpc_profile_refs] = true
...
func (obj *TelemetryProfile) ClearGrpcProfile() {
        if obj.valid[telemetry_profile_grpc_profile_refs] &&
           !obj.modified[telemetry_profile_grpc_profile_refs] {
                obj.storeReferenceBase("grpc-profile", obj.grpc_profile_refs)
        }
        obj.grpc_profile_refs = make([]contrail.Reference, 0)
        obj.valid[telemetry_profile_grpc_profile_refs] = true
        obj.modified[telemetry_profile_grpc_profile_refs] = true
}

func (obj *TelemetryProfile) SetGrpcProfileList(
        refList []contrail.ReferencePair) {
        obj.ClearGrpcProfile()
        obj.grpc_profile_refs = make([]contrail.Reference, len(refList))
        for i, pair := range refList {
                obj.grpc_profile_refs[i] = contrail.Reference {
                        pair.Object.GetFQName(),
                        pair.Object.GetUuid(),
                        pair.Object.GetHref(),
                        pair.Attribute,
...
func (obj *TelemetryProfile) readNetconfProfileRefs() error {
        if !obj.IsTransient() &&
                !obj.valid[telemetry_profile_netconf_profile_refs] {
                err := obj.GetField(obj, "netconf_profile_refs")
                if err != nil {
                        return err
                }
        }
...
func (obj *TelemetryProfile) GetNetconfProfileRefs() (
        contrail.ReferenceList, error) {
        err := obj.readNetconfProfileRefs()
        if err != nil {
                return nil, err
        }
        return obj.netconf_profile_refs, nil
}

func (obj *TelemetryProfile) AddNetconfProfile(
        rhs *NetconfProfile) error {
        err := obj.readNetconfProfileRefs()
        if err != nil {
                return err
        }

        if !obj.modified[telemetry_profile_netconf_profile_refs] {
                obj.storeReferenceBase("netconf-profile", obj.netconf_profile_refs)
        }

        ref := contrail.Reference {
                rhs.GetFQName(), rhs.GetUuid(), rhs.GetHref(), nil}
        obj.netconf_profile_refs = append(obj.netconf_profile_refs, ref)
        obj.modified[telemetry_profile_netconf_profile_refs] = true
        return nil
}

func (obj *TelemetryProfile) DeleteNetconfProfile(uuid string) error {
        err := obj.readNetconfProfileRefs()
        if err != nil {
                return err
        }
        ...
        for i, ref := range obj.netconf_profile_refs {
                if ref.Uuid == uuid {
                        obj.netconf_profile_refs = append(
                                obj.netconf_profile_refs[:i],
                                obj.netconf_profile_refs[i+1:]...)
                        break
                }
        }
        obj.modified[telemetry_profile_netconf_profile_refs] = true
...
func (obj *TelemetryProfile) ClearNetconfProfile() {
        if obj.valid[telemetry_profile_netconf_profile_refs] &&
           !obj.modified[telemetry_profile_netconf_profile_refs] {
                obj.storeReferenceBase("netconf-profile", obj.netconf_profile_refs)
        }
        obj.netconf_profile_refs = make([]contrail.Reference, 0)
        obj.valid[telemetry_profile_netconf_profile_refs] = true
        obj.modified[telemetry_profile_netconf_profile_refs] = true
}

func (obj *TelemetryProfile) SetNetconfProfileList(
        refList []contrail.ReferencePair) {
        obj.ClearNetconfProfile()
        obj.netconf_profile_refs = make([]contrail.Reference, len(refList))
        for i, pair := range refList {
                obj.netconf_profile_refs[i] = contrail.Reference {
                        pair.Object.GetFQName(),
                        pair.Object.GetUuid(),
                        pair.Object.GetHref(),
                        pair.Attribute,
...
func (obj *TelemetryProfile) readSnmpProfileRefs() error {
        if !obj.IsTransient() &&
                !obj.valid[telemetry_profile_snmp_profile_refs] {
                err := obj.GetField(obj, "snmp_profile_refs")
                if err != nil {
                        return err
                }
        }
...
func (obj *TelemetryProfile) GetSnmpProfileRefs() (
        contrail.ReferenceList, error) {
        err := obj.readSnmpProfileRefs()
        if err != nil {
                return nil, err
        }
        return obj.snmp_profile_refs, nil
}

func (obj *TelemetryProfile) AddSnmpProfile(
        rhs *SnmpProfile) error {
        err := obj.readSnmpProfileRefs()
        if err != nil {
                return err
        }

        if !obj.modified[telemetry_profile_snmp_profile_refs] {
                obj.storeReferenceBase("snmp-profile", obj.snmp_profile_refs)
        }

        ref := contrail.Reference {
                rhs.GetFQName(), rhs.GetUuid(), rhs.GetHref(), nil}
        obj.snmp_profile_refs = append(obj.snmp_profile_refs, ref)
        obj.modified[telemetry_profile_snmp_profile_refs] = true
        return nil
}

func (obj *TelemetryProfile) DeleteSnmpProfile(uuid string) error {
        err := obj.readSnmpProfileRefs()
        if err != nil {
                return err
        }
        ...
        for i, ref := range obj.snmp_profile_refs {
                if ref.Uuid == uuid {
                        obj.snmp_profile_refs = append(
                                obj.snmp_profile_refs[:i],
                                obj.snmp_profile_refs[i+1:]...)
                        break
                }
        }
        obj.modified[telemetry_profile_snmp_profile_refs] = true
...
func (obj *TelemetryProfile) ClearSnmpProfile() {
        if obj.valid[telemetry_profile_snmp_profile_refs] &&
           !obj.modified[telemetry_profile_snmp_profile_refs] {
                obj.storeReferenceBase("snmp-profile", obj.snmp_profile_refs)
        }
        obj.snmp_profile_refs = make([]contrail.Reference, 0)
        obj.valid[telemetry_profile_snmp_profile_refs] = true
        obj.modified[telemetry_profile_snmp_profile_refs] = true
}

func (obj *TelemetryProfile) SetSnmpProfileList(
        refList []contrail.ReferencePair) {
        obj.ClearSnmpProfile()
        obj.snmp_profile_refs = make([]contrail.Reference, len(refList))
        for i, pair := range refList {
                obj.snmp_profile_refs[i] = contrail.Reference {
                        pair.Object.GetFQName(),
                        pair.Object.GetUuid(),
                        pair.Object.GetHref(),
                        pair.Attribute,
...
func (obj *TelemetryProfile) MarshalJSON() ([]byte, error) {
        ...
        if len(obj.grpc_profile_refs) > 0 {
                var value json.RawMessage
                value, err := json.Marshal(&obj.grpc_profile_refs)
                if err != nil {
                        return nil, err
                }
                msg["grpc_profile_refs"] = &value
        }

        if len(obj.netconf_profile_refs) > 0 {
                var value json.RawMessage
                value, err := json.Marshal(&obj.netconf_profile_refs)
                if err != nil {
                        return nil, err
                }
                msg["netconf_profile_refs"] = &value
        }

        if len(obj.snmp_profile_refs) > 0 {
                var value json.RawMessage
                value, err := json.Marshal(&obj.snmp_profile_refs)
                if err != nil {
                        return nil, err
                }
                msg["snmp_profile_refs"] = &value
        }
...
func (obj *TelemetryProfile) UnmarshalJSON(body []byte) error {
        ...
        for key, value := range m {
                ...
                case "sflow_profile_refs":
                        ...
                        break
                case "grpc_profile_refs":
                        err = json.Unmarshal(value, &obj.grpc_profile_refs)
                        if err == nil {
                                obj.valid[telemetry_profile_grpc_profile_refs] = true
                        }
                        break
                case "netconf_profile_refs":
                        err = json.Unmarshal(value, &obj.netconf_profile_refs)
                        if err == nil {
                                obj.valid[telemetry_profile_netconf_profile_refs] = true
                        }
                        break
                case "snmp_profile_refs":
                        err = json.Unmarshal(value, &obj.snmp_profile_refs)
                        if err == nil {
                                obj.valid[telemetry_profile_snmp_profile_refs] = true
                        }
                        break
...
func (obj *TelemetryProfile) UpdateObject() ([]byte, error) {
        ...
        if obj.modified[telemetry_profile_grpc_profile_refs] {
                if len(obj.grpc_profile_refs) == 0 {
                        ...
                        msg["grpc_profile_refs"] = &value
                } else if !obj.hasReferenceBase("grpc-profile") {
                        var value json.RawMessage
                        value, err := json.Marshal(&obj.grpc_profile_refs)
                        if err != nil {
                                return nil, err
        ...
        if obj.modified[telemetry_profile_netconf_profile_refs] {
                if len(obj.netconf_profile_refs) == 0 {
                        ...
                        msg["netconf_profile_refs"] = &value
                } else if !obj.hasReferenceBase("netconf-profile") {
                        var value json.RawMessage
                        value, err := json.Marshal(&obj.netconf_profile_refs)
                        if err != nil {
                                return nil, err
        ...
        if obj.modified[telemetry_profile_snmp_profile_refs] {
                if len(obj.snmp_profile_refs) == 0 {
                        ...
                        msg["snmp_profile_refs"] = &value
                } else if !obj.hasReferenceBase("snmp-profile") {
                        var value json.RawMessage
                        value, err := json.Marshal(&obj.snmp_profile_refs)
                        if err != nil {
                                return nil, err
...
func (obj *TelemetryProfile) UpdateReferences() error {
        ...
        if obj.modified[telemetry_profile_grpc_profile_refs] &&
           len(obj.grpc_profile_refs) > 0 &&
           obj.hasReferenceBase("grpc-profile") {
                err := obj.UpdateReference(
                        obj, "grpc-profile",
                        obj.grpc_profile_refs,
                        obj.baseMap["grpc-profile"])
                if err != nil {
                        return err
                }
        }
        ...
        if obj.modified[telemetry_profile_netconf_profile_refs] &&
           len(obj.netconf_profile_refs) > 0 &&
           obj.hasReferenceBase("netconf-profile") {
                err := obj.UpdateReference(
                        obj, "netconf-profile",
                        obj.netconf_profile_refs,
                        obj.baseMap["netconf-profile"])
                if err != nil {
                        return err
                }
        }
        ...
        if obj.modified[telemetry_profile_snmp_profile_refs] &&
           len(obj.snmp_profile_refs) > 0 &&
           obj.hasReferenceBase("snmp-profile") {
                err := obj.UpdateReference(
                        obj, "snmp-profile",
                        obj.snmp_profile_refs,
                        obj.baseMap["snmp-profile"])
                if err != nil {
                        return err
                }
        }
