...
const (
	...
	virtual_machine_interface_virtual_port_group_refs
...
type VirtualMachineInterface struct {
	...
	virtual_port_group_refs contrail.ReferenceList
...
func (obj *VirtualMachineInterface) readVirtualPortGroupRefs() error {
        if !obj.IsTransient() &&
                !obj.valid[virtual_machine_interface_virtual_port_group_refs] {
                err := obj.GetField(obj, "virtual_port_group_refs")
                if err != nil {
                        return err
                }
        }
...
func (obj *VirtualMachineInterface) GetVirtualPortGroupRefs() (
        contrail.ReferenceList, error) {
        err := obj.readVirtualPortGroupRefs()
        if err != nil {
                return nil, err
        }
        return obj.virtual_port_group_refs, nil
}

func (obj *VirtualMachineInterface) AddVirtualPortGroup(
        rhs *VirtualPortGroup, data VMIVirtualPortGroupAttributes) error {
        err := obj.readVirtualPortGroupRefs()
        if err != nil {
                return err
        }

        if !obj.modified[virtual_machine_interface_virtual_port_group_refs] {
                obj.storeReferenceBase("virtual-port-group", obj.virtual_port_group_refs)
        }

        ref := contrail.Reference {
                rhs.GetFQName(), rhs.GetUuid(), rhs.GetHref(), data}
        obj.virtual_port_group_refs = append(obj.virtual_port_group_refs, ref)
        obj.modified[virtual_machine_interface_virtual_port_group_refs] = true
        return nil
}

func (obj *VirtualMachineInterface) DeleteVirtualPortGroup(uuid string) error {
        err := obj.readVirtualPortGroupRefs()
        if err != nil {
                return err
        }
        ...
        for i, ref := range obj.virtual_port_group_refs {
                if ref.Uuid == uuid {
                        obj.virtual_port_group_refs = append(
                                obj.virtual_port_group_refs[:i],
                                obj.virtual_port_group_refs[i+1:]...)
                        break
                }
        }
        obj.modified[virtual_machine_interface_virtual_port_group_refs] = true
...
func (obj *VirtualMachineInterface) ClearVirtualPortGroup() {
        if obj.valid[virtual_machine_interface_virtual_port_group_refs] &&
           !obj.modified[virtual_machine_interface_virtual_port_group_refs] {
                obj.storeReferenceBase("virtual-port-group", obj.virtual_port_group_refs)
        }
        obj.virtual_port_group_refs = make([]contrail.Reference, 0)
        obj.valid[virtual_machine_interface_virtual_port_group_refs] = true
        obj.modified[virtual_machine_interface_virtual_port_group_refs] = true
}

func (obj *VirtualMachineInterface) SetVirtualPortGroupList(
        refList []contrail.ReferencePair) {
        obj.ClearVirtualPortGroup()
        obj.virtual_port_group_refs = make([]contrail.Reference, len(refList))
        for i, pair := range refList {
                obj.virtual_port_group_refs[i] = contrail.Reference {
                        pair.Object.GetFQName(),
                        pair.Object.GetUuid(),
                        pair.Object.GetHref(),
                        pair.Attribute,
...
func (obj *VirtualMachineInterface) MarshalJSON() ([]byte, error) {
        ...
        if len(obj.virtual_port_group_refs) > 0 {
                var value json.RawMessage
                value, err := json.Marshal(&obj.virtual_port_group_refs)
                if err != nil {
                        return nil, err
                }
                msg["virtual_port_group_refs"] = &value
        }
...
func (obj *VirtualMachineInterface) UnmarshalJSON(body []byte) error {
        ...
        for key, value := range m {
                ...
                case "virtual_port_group_refs": {
                        type ReferenceElement struct {
                                To []string
                                Uuid string
                                Href string
                                Attr VMIVirtualPortGroupAttributes
                        }
                        var array []ReferenceElement
                        err = json.Unmarshal(value, &array)
                        if err != nil {
                        ...
                        obj.valid[virtual_machine_interface_virtual_port_group_refs] = true
                        obj.virtual_port_group_refs = make(contrail.ReferenceList, 0)
                        for _, element := range array {
                                ref := contrail.Reference {
                                        element.To,
                                        element.Uuid,
...
func (obj *VirtualMachineInterface) UpdateObject() ([]byte, error) {
        ...
        if obj.modified[virtual_machine_interface_virtual_port_group_refs] {
                if len(obj.virtual_port_group_refs) == 0 {
                        ...
                        msg["virtual_port_group_refs"] = &value
                } else if !obj.hasReferenceBase("virtual-port-group") {
                        var value json.RawMessage
                        value, err := json.Marshal(&obj.virtual_port_group_refs)
                        if err != nil {
                                return nil, err
...
func (obj *VirtualMachineInterface) UpdateReferences() error {
        ...
        if obj.modified[virtual_machine_interface_virtual_port_group_refs] &&
           len(obj.virtual_port_group_refs) > 0 &&
           obj.hasReferenceBase("virtual-port-group") {
                err := obj.UpdateReference(
                        obj, "virtual-port-group",
                        obj.virtual_port_group_refs,
                        obj.baseMap["virtual-port-group"])
                if err != nil {
                        return err
                }
        }
